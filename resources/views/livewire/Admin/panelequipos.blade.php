    ...
    <div class="min-h-screen bg-gray-50">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                            <span class="text-white font-medium">{{ $equipos->count() }} equipos registrados</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex justify-between items-center mb-8">
                ...
                <div class="flex items-center space-x-4">
                    <div class="relative">
                        <input type="text" wire:model.live="searchQuery1" placeholder="Buscar equipos..."
                        ...
                        <div class="bg-white rounded-xl card-shadow hover-lift overflow-hidden animate-fade-in">
                            ...
                            <div class="h-48 bg-gradient-to-br from-purple-400 to-blue-500 relative overflow-hidden">
                                @if ($equipo->Det_equipo->imagenes_equipo)
                                    <img src="{{ asset('storage/' . $equipo->Det_equipo->imagenes_equipo) }}"
                                        alt="{{ $equipo->Det_equipo->name_equipo }}" class="w-full h-full object-cover">
                                @else
                                ...
                                <div class="absolute top-4 left-4">
                                    <span
                                        ...
                                        {{ $equipo->Det_equipo->categoria->namecategorias ?? 'Sin categoría' }}
                                    </span>
                                </div>
                            ...
                            <div class="p-6">
                                <div class="flex justify-between items-start mb-3">
                                    <h3 class="text-xl font-bold text-gray-900">{{ $equipo->Det_equipo->name_equipo }}
                                    </h3>
                                ...
                                <div class="flex items-center text-green-600 mb-3">
                                    ...
                                    <span class="text-lg font-bold">S/ {{ $equipo->Det_equipo->precio_equipo }}</span>
                                </div>

                                <p class="text-gray-600 text-sm mb-4 line-clamp-3">
                                    {{ Str::limit($equipo->Det_equipo->descripcion_equipo ?? 'Sin descripción disponible', 120) }}
                                </p>

                                <div class="space-y-2">
                                    @if ($equipo->Det_equipo->marca)
                                        <div class="flex items-center text-sm text-gray-500">
                                            ...
                                            <span>{{ $equipo->Det_equipo->marca->namemarcas }}</span>
                                        </div>
                                    @endif

                                    @if ($equipo->Det_equipo->modelo)
                                        <div class="flex items-center text-sm text-gray-500">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span>{{ $equipo->Det_equipo->modelo->namemodelos }}</span>
                                        </div>
                                    @endif

                                    @if ($equipo->Det_equipo->serie)
                                        <div class="flex items-center text-sm text-gray-500">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span>Serie: {{ $equipo->Det_equipo->serie->nameserie }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select wire:model="form.fk_idcategoria"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('form.fk_idcategoria') border-red-500 @enderror">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model="form.fk_idserie"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('form.fk_idserie') border-red-500 @enderror">
                                                <option value="">-- Selecciona una serie --</option>
                                                @foreach ($series as $serie)
                                                    <option value="{{ $serie->id }}">{{ $serie->nameserie }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('form.fk_idserie')
                                                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select wire:model="form.fk_idmarca"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('form.fk_idmarca') border-red-500 @enderror">
                                                <option value="">-- Selecciona una marca --</option>
                                                @foreach ($marcas as $marca)
                                                    <option value="{{ $marca->id }}">{{ $marca->namemarcas }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('form.fk_idmarca')
                                                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select wire:model="form.fk_idmodelo"
                                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('form.fk_idmodelo') border-red-500 @enderror">
                                        ...
                                        <div class="mb-4">
                                            @if ($form['imagenes_equipo'])
                                                <div class="relative">
                                                    ...
                                                    <button type="button"
                                                        wire:click="$set('form.imagenes_equipo', null)"
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        ¿Estás seguro de que deseas eliminar este equipo? Esta acción no se puede
                                        deshacer.
                                    </p>
                                </div>
                            </div>
